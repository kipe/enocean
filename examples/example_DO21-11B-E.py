...
def send_command(destination, output_value):
    global communicator
    communicator.send(
        RadioPacket.create(EEP(), rorg=RORG.VLD, rorg_func=0x01, rorg_type=0x01, destination=destination, sender=communicator.base_id, command=1, IO=0x1E, OV=output_value)
    )
